...
Before we set out to gather/import data from diverse sources, we should start organizing the procedure in an R script. This script will be the begining of our pipeline!
...
RStudio recognizes different sections in a script, whereby section headers are indicated by ` ----------`. This helps to further organize the script into different tasks. Usually, it makes sense to start with a 'meta' section in which all necessary packages are loaded and fix variables are initiated.

...
# Download data
URL <- 'https://docs.google.com/spreadsheets/d/1P_z7HbNrsif3U2RLR0BLNXiaojcTPCm2KpgbvPVdZ6k/edit?usp=sharing'
...
# Majors
majors <- c('BWL', 'VWL', 'BIA', 'BLaw', 'BLE','other')
...
register_google(key = "AIzaSyCy1Gjqz-WNaXxtrNLrfMoSLLqOKF3Q6_Q")
hometown_coords <- geocode(hometowns[c(1,length(hometowns))]) # problem: might reach limit!
...
# Plot network
simpleNetwork(distances[, 1:2], zoom = TRUE, linkColour = "#02934a", nodeColour = "#02934a")
```
...
renderPlot({
    # Subset data
    data_sub <- subset(student_data, majors == input$majors)
